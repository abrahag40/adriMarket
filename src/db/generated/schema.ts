import { pgTable, index, foreignKey, bigserial, uuid, text, jsonb, timestamp, uniqueIndex, boolean, date, check, integer, bigint, char, inet, numeric, unique, smallint, time, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const bookingItems = pgTable("booking_items", {
	...
	check("booking_items_shape", sql`((kind = ANY (ARRAY['stay'::product_kind, 'vehicle'::product_kind])) AND (rental_unit_id IS NOT NULL) AND (rental_range IS NOT NULL) AND (guests IS NOT NULL) AND (tour_departure_id IS NULL) AND (seats IS NULL)) OR ((kind = 'tour'::product_kind) AND (tour_departure_id IS NOT NULL) AND (seats IS NOT NULL) AND (rental_unit_id IS NULL) AND (rental_range IS NULL))`),
...
export const payments = pgTable("payments", {
	...
	index("payments_booking_idx").using("btree", table.bookingId.asc().nullsLast().op("uuid_ops"), table.purpose.asc().nullsLast().op("uuid_ops")),
...
export const bookings = pgTable("bookings", {
	...
	index("bookings_status_idx").using("btree", table.status.asc().nullsLast().op("timestamptz_ops"), table.createdAt.desc().nullsFirst().op("timestamptz_ops")),
	foreignKey({
...
export const refunds = pgTable("refunds", {
	...
	index("refunds_pendientes_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamptz_ops")).where(sql`(status = 'pending'::payment_status)`),
	index("refunds_settled_idx").using("btree", table.settledAt.asc().nullsLast().op("timestamptz_ops"), table.method.asc().nullsLast().op("timestamptz_ops")).where(sql`(settled_at IS NOT NULL)`),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [staffUsers.id],
	...
	foreignKey({
			...
			foreignColumns: [staffUsers.id],
			name: "refunds_settled_by_fkey"
		}),
	check("refunds_amount_cents_check", sql`amount_cents > 0`),
	check("refunds_succeeded_esta_explicado", sql`(status <> 'succeeded'::payment_status) OR ((settled_at IS NOT NULL) AND (method IS NOT NULL) AND (settled_by IS NOT NULL))`),
]);
